...
if __name__ == '__main__':
    ...
    #             embedding_dim=100,
    #              seed=SEED)
    dict = np.load('./data/demostration.npy', allow_pickle=True).item()

    disc= Discriminator(EMBEDDING_DIM+3*EMBEDDING_DIM, subsampling_rate = env.max_episodes,lambd = 10.0, gail_loss='gail')
    algo = InverseDDPG(action_shape = EMBEDDING_DIM,seed = SEED,disc = disc,dict = dict,epoch_disc = 1, gamma = 0.9, ac_lr = 1e-3, cr_lr = 1e-3,tau = 0.001,
    ...
    recommender = Trainer(env, env, algo, users_num, items_num,
                 use_wandb= False, load=False, load_step=100000,
                 load_memory= False, seed=0, num_steps= NUMBER_STEPS,
                 eval_interval= 500, num_eval_episodes=2,num_steps_before_train= 800,
                 model_dir = './model', memory_dir = './memory',top_k = False)

    recommender.train_imitation()
    # print("done!")